/**
 * TranscriptView component - displays and edits meeting transcript
 */

import React, { useState } from 'react';
import { Download, RefreshCw, RotateCcw, Edit } from 'lucide-react';
import { TranscriptEntry, TranscriptSaveStatus, SpeakerSuggestions, SpeakerMapping } from '../types';

interface TranscriptViewProps {
  transcript: TranscriptEntry[];
  originalTranscript: TranscriptEntry[];
  selectedMeetingId: string | null;
  speakerNameMaps: Record<string, SpeakerMapping>;
  speakerSuggestions: SpeakerSuggestions | null;
  speakerIdentificationLoading: boolean;
  transcriptSaveStatus: TranscriptSaveStatus;
  currentSpeakerNameMap: SpeakerMapping;
  onSpeakerNameChange: (originalSpeaker: string, newName: string) => void;
  onTranscriptTextChange: (entryId: string, newText: string) => void;
  onIdentifySpeakers: (meetingId: string) => void;
  onExportTranscript: () => void;
  onResetTranscript: () => void;
  onApplySpeakerSuggestion: (originalSpeaker: string, suggestedName: string) => void;
  onDismissSpeakerSuggestion: (speaker: string) => void;
  getSpeakerColor: (speakerId: number) => string;
  formatSpeakerName: (speakerName: string) => string;
...
const TranscriptView: React.FC<TranscriptViewProps> = ({
  ...
  const [editingTranscriptEntry, setEditingTranscriptEntry] = useState<string | null>(null);

  const toggleTextEditing = (entryId: string) => {
    setEditingTranscriptEntry(
      editingTranscriptEntry === entryId ? null : entryId
    );
  };

  const handleTranscriptTextEdit = (entryId: string, newText: string | null) => {
    if (onTranscriptTextChange && newText) {
              ...
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !e.shiftKey) {
                    ...
                    e.currentTarget.textContent = entry.text; // Restore original text
                    setEditingTranscriptEntry(null);
                  }
                }}
