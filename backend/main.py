...
try:
    ...
    logger.info(f"Timezone configured: GMT{timezone_offset_str} ({timezone_offset_hours} hours)")
except (ValueError, TypeError) as e:
    logger.warning(f"Invalid TIMEZONE_OFFSET '{os.getenv('TIMEZONE_OFFSET')}', using default GMT+8. Error: {e}")
...
def generate_professional_pdf(
    ...
    if summary_data:
        ...
        meeting_info = [
            ...
            ['Generated On:', generated_on or datetime.now(tz_configured).strftime('%B %d, %Y at %I:%M %p')],
...
async def update_summary(uuid: str, request: dict) -> SummaryResponse:
    ...
    logger.info(f"Updated summary for {uuid}")
...
async def export_transcript_pdf(uuid: str, request: ExportRequest = None):
    ...
    try:
        ...
        logger.error(f"Error generating transcript PDF: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error during transcript PDF generation")
...
async def export_transcript_markdown(uuid: str, request: ExportRequest = None):
    ...
    try:
        ...
        logger.error(f"Error generating transcript markdown: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error during transcript markdown generation")
